let rec fold_int i n c f = 
  if i < n then fold_int (i+1) n (f i c) f
  else c
...
module Grammar (Tree: Tree) (Nonterm: Nonterminal) (Attr: Attribute) =
  struct

    module WrappedNonterm (Nonterms: Nonterminal) =
      struct
        ...
        module Verify =
          struct
            ...
            let run () =
              begin try
                let _ = fold_int 0 number () (fun i _ -> void (Nonterms.toString i)) 
                in Yes
    ...
    module WrappedTree (PTree: Tree) =
      struct
        ...
        let toString' (Foo (t, i, _)) = 
          (string_of_int i) ^ ": " ^ (PTree.toString t)
        ...
        module Verify =
          struct
            ...
            let run () =
              begin try
                begin match 
                  fold_int 0 PTree.terminals [] 
                    (fun i c ->
                      if i <> PTree.term (PTree.node i) 
                      then (PTree.toString (PTree.node i), i) :: c
                      else c 
                    )  with
                | [] -> Yes
                | x  -> No x
        ...
        let build rules =
          begin try
            Ok ((
             ...
             rules (Tree.terminals, Nonterm.number, Tree.degree)
            )
          with 
          | Terminal (n, lab) -> 
              Fail ("wrong terminal #" ^ (string_of_int n) ^ " encountered in rule \"" ^ lab ^ "\"")
		
          | Nonterm (n, lab) -> 
              Fail ("wrong nonterminal #" ^ (string_of_int n) ^ " encountered in rule \"" ^ lab ^ "\"")
		
          | Derived (n, lab) -> 
              Fail ("wrong derived nonterminal #" ^ (string_of_int n) ^ " encountered in rule \"" ^ lab ^ "\"")
		
          | Degree (n, lab) -> 
              Fail ("wrong degree " ^ (string_of_int n) ^ " encountered in rule \"" ^ lab ^ "\"")
        ...
        let toString = function 
          | Ok ({normals=norm; chains=ch}) -> 
              ...
              let by_degree a = 
                fst (fold_left 
		       (fun (curr, i) r -> 
			 curr ^ "\n" ^ "      degree=" ^ (string_of_int i) ^ (rules r), i+1
		       ) 
                       ("" , 0) 
                       a
                    )
              ...
              let by_nonterm a = 
                fst (fold_left 
		       (fun (curr, i) r -> 
			 curr ^ "\n" ^ "    nonterm=" ^ (Nonterm.toString i) ^ (rules r), i+1
		       ) 
                       ("" , 0) 
                       a
                    )
              ...
              fst (fold_left 
		     (fun (curr, i) r ->
			curr ^ "\n" ^ "    terminal=" ^ (Tree.toString (Tree.node i)) ^
			(by_degree r), i+1
                     )
                     ("", 0)
                     norm
    ...
    module Labeler =
      struct
              ...
              let rec inner ofs curr (WTree.Foo (_, inode, sons) as t) =
                List.fold_left 
                  ...
                  ((String.make ofs ' ') ^ (WTree.toString' t) ^ ": " ^
                   (fold_int 0 WNonterm.number "" 
                      (fun i curr ->
                        curr ^  
                        match table.(inode).(i) with 
                        | Some {attr=a; cost=c; rule=(_, lab)} ->
                            (WNonterm.toString i) ^ 
                            " (attr=" ^ (Attr.toString  a) ^ 
                            "; cost=" ^ (string_of_int c) ^ 
                            "; rule=\"" ^ lab ^ "\"); "
                        | None -> ""
                      )
                   ) ^ "\n"
        ...
        let run' (root, nnodes) = function 
          ...
          | Table.Ok   table  ->
             let labels = init nnodes (fun _ -> init WNonterm.number (fun _ -> None)) in
        
             let rec process_node node =

               let term, degree = WTree.term node, List.length (WTree.sons node) in
                   ...
                   let attrs, costs = 
                     List.fold_left2 
                       (fun (al, cl) node nonterm ->
                         ...
                         | None                          -> raise Break
                         | Some {attr=a; cost=c; rule=_} -> (a :: al, c :: cl)  
                       )
                       ([], [])
                       nodes
                       nonterms      
                   in Some (List.rev attrs, List.rev costs)
                 with 
                 | Break -> None
                 | x     -> raise x
                 end
		 ...
		 let cost        = Attr.cost attr in
                 if pred tree attrs then
                   ...
                   | None ->
                       labels.(inode).(derived) <- Some {attr=attr; cost=cost; rule=rule};
               ...
               and build_closure node nonterm =
                 ...
                 List.iter 
                   (fun ((Chain (d, n, pred, sema, _), _) as rule) ->
                     let Some {attr=a; cost=c; rule=_} = labels.(i).(n) in
               ...
               let apply_rules rules node =
                 List.iter 
                   (fun ((Normal (d, _, nonterms, pred, sema, _), _) as rule) ->
                     ...
                     | Some (attrs, costs) -> apply_rule node d pred sema attrs rule
                     | None                -> ()
                   )
                   rules;
                 try
                   iter (fun e -> match e with Some _ -> raise Break | _ -> ()) labels.(WTree.id node);
	...
	let rec reduce node nonterm labels =
          ...
          let Some {Labeler.attr=attr; Labeler.cost=_; Labeler.rule=rule} = labels.(i).(nonterm) in
		...
		List.split 
	      ...
	      in
              sema t attr (List.map2 (fun node' nonterm' -> reduce node' nonterm' labels) sons sons')
        ...
        let run = function
          ...
          | Labeler.Ok   (labels, tree) ->
              Ok (reduce 
			    ...
			    (fun (curr, (max, i)) lab -> 
			      match lab with
			      | Some {Labeler.attr=_; Labeler.cost=c; Labeler.rule=_} when c < max -> 
				  i, (c, i + 1)
			      | _ -> 
				  curr, (max, i + 1)
			    ) 
    ...
    module Verify =
      struct
        ...
        let run () =
          begin match WTree.Verify.run (), WNonterm.Verify.run () with
          | WTree.Verify.Yes, WNonterm.Verify.Yes -> Yes
          | tree            , nont                -> No (
              "tree verification: " ^ (WTree.Verify.toString tree) ^ ", " ^
              "nonterminals verification: " ^ (WNonterm.Verify.toString nont) 
             )
          end
...
module BURS (Tree: Tree) (Nonterm: Nonterminal) =
  struct
    ...
    let rule_p (n, t, l, p, s, r) = Grammar.rule_p (n, t, l, p, s, (fun x y z -> r x z))
...
module ConstBURS (Tree: Tree) (Nonterm: Nonterminal) =
  struct
